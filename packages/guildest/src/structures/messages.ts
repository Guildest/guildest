...
export type ApiPartialMessageResolve = ApiMessage | wsMessageDelete;

export class Message extends Base<ApiPartialMessageResolve> {
	type?: ApiMessageType;
	serverId?: string;
	channelId: string;
	content?: string;
	...
	updatedAt?: number;
	constructor(client: Client, json: ApiPartialMessageResolve) {
		super(client, json);
	...
	_update(json: Partial<ApiPartialMessageResolve>) {
		if ('type' in json) this.type = json.type;
		if ('createdAt' in json) this.createdAt = DateParse(json.createdAt);
		if ('createdBy' in json) this.createdBy = json.createdBy;
